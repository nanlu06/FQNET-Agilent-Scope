...
dpo.write(':STOP;*OPC?')
print("test---1")
dpo.write(':SYSTem:SETup?')
dpo.writeIEEEBlock ':SYSTem:SETup', varSetup
...
#	print "OPC is ",dpo.query('*OPC?')
	if (int(dpo.query(':ADER?')) == 1): 
                ...
                print 'Timestamp for acquisition completion ', StopTime    
                TimestampFileHandle = open(TimestampFilePath,"a+")
                TimestampFileHandle.write(str(runNumber) + "\t" + str(StopTime)+ "\t"+ str(time.time()) + "\n")
...
dpo.write('MEASure:DELTatime: CHANnel1, CHANnel2')
VarValue=dpo.ReadNumber
print("time resolution: ".VarValue)
